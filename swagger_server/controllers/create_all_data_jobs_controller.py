...
def data_jobs_post(body):  # noqa: E501
    ...
    try:
        ...
        # 응답 본문에 dataJobInfo만 포함하고, Location 헤더에 data_job_id 추가
        response = jsonify({
            "dataJobInfo": body.to_dict(),  # 응답 본문에 dataJobInfo만 포함
        })
        
        # Location 헤더에 data_job_id 추가
        response.headers['Location'] = f"/data-access/v1/data-jobs/{data_job_id}"

        return response, 201  # HTTP 201 Created 응답과 함께 반환
    
    except Exception as e:
        # 예외 처리: 요청 본문이 잘못된 경우 400 오류 반환
